        ...
        <div className="flex flex-col gap-4 md:flex-row">
          ...
          <div className="flex w-full flex-col md:w-1/3">
            <div className="flex w-full flex-row items-center justify-between">
              ...
              <CardDescription className="outfit-uniquifier text-md">
                {data?.[0]?.year ?? "Laddar..."}
              </CardDescription>
            </div>
            <CardDescription className="merriweather-long-text text-md mt-2">
              {data?.[0]?.description ?? "Laddar..."}
            </CardDescription>
            <CardDescription className="outfit-uniquifier mt-2 flex flex-row items-center gap-2">
              <Badge variant="outline" className="text-sm">
                {data?.[0]?.price ?? "Laddar..."}kr
              </Badge>
            </CardDescription>
            <CardFooter className="outfit-uniquifier mt-auto flex w-full justify-end px-0">
              <Link
                href={`/art/${data?.[0]?.id ?? ""}`}
